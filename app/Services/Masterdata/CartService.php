...
class CartService
{
    ...
    public function currentCart($websiteId, bool $create = false): ?Cart
    {
        if (!$create) {
            ...
            return $token
                ? Cart::forWebsite($websiteId)->where('cart_token', $token)->first()
                : null;
        }
    ...
    public function rawLines($websiteId): array
    {
        ...
        return $cart->items->map(fn ($i) => ['id' => $i->item_key, 'qty' => (int) $i->qty])->all();
    }
        ...
        DB::transaction(function () use ($cart, $rows) {
            ...
            foreach ($rows as $r) {
                CartItem::updateOrCreate(
                    ['cart_id' => $cart->id, 'item_key' => $r['key']],
                    [
                        'item_type'          => $r['kind'],
            ...
            CartItem::where('cart_id', $cart->id)->whereNotIn('item_key', $keep)->delete();
        });
    ...
    public function resolveItems($websiteId, array $lines): array
    {
        ...
        $products = array_intersect(['product', 'variant'], $kinds)
            ? Product::whereIn('id', $idsOf(['product', 'variant']))
                ->whereHas('service', fn ($q) => $q->where('website_id', $websiteId))
        ...
        $packages = in_array('package', $kinds, true)
            ? Package::whereIn('id', $idsOf(['package']))
        ...
        $services = in_array('service', $kinds, true)
            ? Service::whereIn('id', $idsOf(['service']))
                ->where('website_id', $websiteId)
                ->where('is_active', true)
                ->get()->keyBy('id')
            : collect();
